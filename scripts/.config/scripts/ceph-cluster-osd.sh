...
if ! tmux has-session -t $SESSION_NAME 2>/dev/null; then
    ...
    # 4. Send SSH commands
    tmux send-keys -t "${PANES[0]}" 'ssh ceph-osd-1' C-m
    tmux send-keys -t "${PANES[1]}" 'ssh ceph-osd-2' C-m
    tmux send-keys -t "${PANES[2]}" 'ssh ceph-osd-3' C-m
    tmux send-keys -t "${PANES[3]}" 'ssh ceph-osd-4' C-m
